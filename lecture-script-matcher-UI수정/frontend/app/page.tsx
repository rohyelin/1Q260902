...
import { createJob, createTranscriptJob } from "@/lib/api";
...
export default function HomePage() {
  ...
  const handleSubmit = async () => {
    ...
    if (!primaryFile || !pdfFile || startedRef.current) return;
    ...
    try {
      ...
      const { job_id } =
        inputMode === "audio"
          ? await createJob(audioFile as File, pdfFile, options)
          : await createTranscriptJob(transcriptFile as File, pdfFile, options);
      setProcessing(true);
      ...
      {!processing && (
        <section className="max-w-5xl mx-auto px-6 pt-20 pb-4 text-center">
          ...
          <p className="font-reading mx-auto mt-4 max-w-[30em] text-[15px] leading-[1.9] text-black/50">
            강의 녹음과 강의록을 올리면, 슬라이드마다 교수님이 하신 말을 붙여
            정리본으로 만들어 드립니다.
          </p>
          <div className="mx-auto mt-7 h-px w-14 bg-black/15" />
        </section>
      ...
      <div className="max-w-5xl mx-auto px-6 pt-10 md:pt-12 pb-16">
        ...
        >
          ...
          <ol className="list-decimal list-inside space-y-1.5">
            <li>상단에서 <b>녹음본 업로드</b> 또는 <b>전사본 업로드</b>를 고릅니다.</li>
            <li>녹음본이면 강의 음성을, 전사본이면 텍스트 스크립트를 올립니다.</li>
          ...
          <p className="text-[13px] text-black/40">
            ※ 전사본을 올리면 음성 전사·용어 교정·가독성 단계를 건너뛰고 바로 슬라이드 매칭부터 시작합니다.
          </p>
        </section>
        ...
        >
          ...
          <p>지원 음성: mp3, m4a, wav, mp4, webm · 전사본: txt, md, 워드(docx), pdf, vtt, srt · 강의록: PDF</p>
