...
/** 정리 항목. 분량이 많으면 한 단계씩 조여 장 안에 담는다. */
const BULLET_DENSITY: Record<Density, string> = {
  loose: "mb-2 leading-[1.7] text-[11px]",
  mid: "mb-1.5 leading-[1.6] text-[10px]",
  tight: "mb-1 leading-[1.5] text-[9px]",
};
...
function Bullet({
  ...
  return (
    <li className={`flex gap-2 last:mb-0 ${BULLET_DENSITY[density]}`}>
...
function SlideSection({
  ...
  // 왼쪽 칸은 강의록과 핵심 정리가 높이를 나눠 쓴다. 정리가 길면 강의록을
  // 줄여 자리를 내주지 않으면 정리 끝이 잘린다 (장 높이가 174mm로 고정이라).
  const summaryChars = summary.reduce((n, s) => n + s.length, 0);
  const summaryDensity: Density =
    summaryChars > 800 ? "tight" : summaryChars > 450 ? "mid" : "loose";
  const imageMax =
    summaryChars > 900
      ? "62mm"
      : summaryChars > 600
  ...
  return (
    ...
    <article className="print-slide break-after-page last:break-after-auto grid w-full grid-cols-[60fr_40fr] gap-[7mm]">
      {/* 왼쪽 6 */}
      <div className="flex min-w-0 flex-col gap-[5mm] overflow-hidden">
        {imageUrl && (
          <figure className="m-0 shrink-0 break-inside-avoid">
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img
              src={imageUrl}
              alt={`슬라이드 ${page.page}`}
              className="block w-full border border-black/10 object-contain"
              style={{ maxHeight: imageMax }}
            />
          </figure>
        )}

        <p className="font-sans text-[9px] uppercase tracking-[0.14em] opacity-45 shrink-0">
          슬라이드 {page.page} / {total}
        </p>

        {summary.length > 0 && (
          <section
            className="min-h-0 flex-1 overflow-hidden rounded-lg px-[7mm] py-[6mm]"
            style={boxColor ? { backgroundColor: boxColor } : undefined}
          >
            <SectionLabel>핵심 정리</SectionLabel>
            <ul>
              {summary.map((raw, i) => (
                <Bullet key={i} density={summaryDensity}>
                  {raw.replace(/^[-•*]\s*/, "")}
                </Bullet>
              ))}
            </ul>
